...
Rotation anchors use inset corner positions while the selected on-screen bounds are at least 90 pixels on both axes. `rotate.svg` is authored for the top-right corner and mirrored for the other corners; all rotation artwork has a 1-pixel canvas-dark outline. Below the threshold, anchors move 14 screen pixels outside the centers of the four sides and render the 20-pixel-wide `rotate_distant.svg` artwork, mirrored outward for each side, while the 14-pixel hit radius remains unchanged. The rotation hover cursor is an 8-pixel white dot with a dark rim. Apply the same adaptive rule to group controls. Show the movement cursor only for selected objects.
