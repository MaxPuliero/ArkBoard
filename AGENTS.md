...
Optional edge snapping is implemented in `BoardSurface` and starts disabled. Move and proportional-scale snapping use the transformed visible bounds, so masks define the active edge. The eight-pixel screen threshold is converted to world units using the current zoom, and active matches render as constant-width white screen-space guides.
