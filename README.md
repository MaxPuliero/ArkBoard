...
## Screen capture fix in 1.18.1

- The camera button now keeps the selection overlay open when ArkBoard remains visible. Screen capture no longer cancels immediately after clicking the button.

## Screen capture and canvas controls in 1.18.0

- Click the camera button below **Aa**, then drag a rectangle across the desktop. ArkBoard remains visible while selecting. Click the canvas to place the captured image; press **Esc** during selection or before placement to cancel. Captures are embedded in the project.
- Rotation anchors use mirrored SVG artwork at image corners and dedicated outward-facing artwork at small zoom levels. Their hover cursor is a small white dot.
- The shared loading and saving progress bar sits 100 pixels higher for better visibility. The application logo has also been refreshed.

## Selection-focused view controls in 1.17.1

- Double-click an image to select and fit it. Press **F** to fit the current selection, including multiple selected images; with nothing selected, **F** fits the whole board.

## Update checking in 1.17.0

- ArkBoard checks the latest GitHub release asynchronously after startup and shows a non-blocking dark notification only when a newer version is available.
- **Settings → Auto-Check for Updates** enables or disables the startup check. Use the adjacent **Check** button to search immediately; available releases can be downloaded to a location chosen with the Windows Save dialog. The preference is stored locally in `%APPDATA%\ArkBoard\settings.ini` and defaults to enabled.
...
- Project loading now runs in the background and shares the compact bottom progress indicator used by saving. The indicator remains visible when the rest of the interface is hidden.
- **Open** opens the standard file dialog. **Open Recent** is a separate submenu in both the File menu and the canvas right-click menu, listing up to ten recent projects. Recent paths are stored locally in `%APPDATA%\ArkBoard\recent-projects.txt`.
- The canvas right-click menu mirrors the complete **File / Edit / View / Settings / Help** menu bar, so every menu command remains available while the interface is hidden.

## Faster background saving in 1.14.0

- Project saving runs on a background task and shows a thin determinate progress bar along the bottom edge of the window.
- PNG, JPEG, GIF, WebP and ICO assets are copied directly into the project archive instead of being redundantly recompressed. Other formats use fast ZIP compression.
- Saves remain atomic. Edits made after the save snapshot stay marked as unsaved, and the progress bar disappears when writing finishes.

## Inline padding and temporary snap override in 1.13.2

- Image Padding now defaults to 4 px and is edited directly inside Settings using a numeric field or the adjacent triangle controls.
- Hold **Ctrl** while moving or scaling to temporarily invert Snapping: Ctrl enables it when the setting is off and suspends it when the setting is on.
- The drag-zoom preference is now labeled **Invert Zoom**.

## Shared image padding in 1.13.1

- **Settings → Image Padding** controls the canvas-pixel gap used by both packing and edge snapping. The default is 16 px.
- Adjacent images snap with the selected gap while same-side edge alignment remains exact.
- The empty-board “A space for your ideas” heading now uses light gray instead of white.
...
- Multiple selected images share a group bounding box with proportional corner scaling and rotation around the common center. Relative spacing, individual proportions, flips and masks are preserved, and the complete transform is undoable in one step.

## BeeRef import in 1.11.0
...
- The toolbar has been removed to give the canvas more space. Right-click the canvas for Import Images, Open, Open Recent, Save Project, Undo, Redo, Fit All and Pack Images, along with the existing editing commands.
